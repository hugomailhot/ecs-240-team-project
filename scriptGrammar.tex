...
\hyperlink{OPSEM}{Section 2} presents a consistent and exhaustive BNF describing Script's syntax.

\hyperlink{OPSEM}{Section 3} specifies the operational semantics of the syntactic constructs defined in \hyperlink{OPSEM}{Section 2}.
\section{Syntax}
...
{	
	<\texttt{PUSH } B, \sigma> \Downarrow \sigma[|S| = L+1, S_{|S|} = B]
}
\vspace{3mm}
...
\inferrule
{
    ...
    B \Downarrow n \hspace{3mm} 
	L \geq n
}
{
	<\texttt{DROP } B, \sigma> \Downarrow \sigma[S_i = S_{i+1} \hspace{3mm} \forall i. \hspace{1mm} n \leq i < L]
}
\vspace{3mm}
...
\inferrule
{|AS| = L}
{<\texttt{PUSH\_ALT } B, \sigma> \Downarrow \sigma[|AS| = L+1, AS_{|AS|} = B]}
\vspace{3mm}
...
\inferrule
{
	\sigma(|AS|) = L \hspace{3mm} \sigma(|AS|) \geq B
}
{
	<\texttt{DROP\_ALT } B, \sigma> \Downarrow \sigma[AS_i = AS_{i+1} \hspace{3mm} \forall i. \hspace{1mm} B \leq i < L]
}
\vspace{3mm}
...
{
	<\texttt{TEST $x$}, \sigma> \Downarrow FALSE
}
\vspace{3mm}
...
If top of stack is present and non-zero, duplicate the top of the stack. \\
...
\section{Operational Semantics -- Small Step Semantics}
 
\subsection{Local reduction rules}

[Have to explain what are atomic commands, and why we're defining them.]
...
<\texttt{com}, \sigma>
\\
&<\texttt{atom\_com}, \sigma[S=s, V=v]> 
	\hspace{3mm}
\longrightarrow 
	\hspace{3mm}
<\texttt{OP\_NOP}, \sigma[S=s', V=v']>
\\
